package com.loreJourney.screen.game;
...
public class TransitionScreen {
    ...
    public void start(EventState prev, EventState next) {
        ...
        switch (transitionIndex) {
            case 0: x = 0; break;
            case 1: y = LoreJourney.V_HEIGHT; break;
            case 2: x0 = 0; x1 = LoreJourney.V_WIDTH; break;
            case 3: y0 = 0; y1 = LoreJourney.V_HEIGHT; break;
            case 4: x0 = 0; x1 = LoreJourney.V_WIDTH; break;
            case 5: y0 = 0; y1 = LoreJourney.V_HEIGHT; break;
        }
    }
    ...
    public void render(float dt) {
        ...
        if (shouldStart) {
            switch (transitionIndex) {
                // horizontal slide l2r
                case 0:
                    shapeRenderer.rect(x, 0, -LoreJourney.V_WIDTH, LoreJourney.V_HEIGHT);
                    break;
                // vertical slide t2b
                case 1:
                    shapeRenderer.rect(0, y, LoreJourney.V_WIDTH, LoreJourney.V_HEIGHT);
                    break;
                // horizontal strips (height: 120 = 8 stripes of height 15)
                case 2:
                    for (int i = 0; i < 8; i++) {
                        if (i % 2 == 0) shapeRenderer.rect(x0, i * 15, -LoreJourney.V_WIDTH, 15);
                        else shapeRenderer.rect(x1, i * 15, LoreJourney.V_WIDTH, 15);
                    }
                    break;
                // vertical strips (width: 200 = 10 stripes of width 20)
                case 3:
                    for (int i = 0; i < 10; i++) {
                        if (i % 2 == 0) shapeRenderer.rect(i * 20, y0, 20, -LoreJourney.V_HEIGHT);
                        else shapeRenderer.rect(i * 20, y1, 20, LoreJourney.V_HEIGHT);
                    }
                    break;
                // horizontal split
                case 4:
                    shapeRenderer.rect(x0, 0, -LoreJourney.V_WIDTH / 2, LoreJourney.V_HEIGHT);
                    shapeRenderer.rect(x1, 0, LoreJourney.V_WIDTH / 2, LoreJourney.V_HEIGHT);
                    break;
                // vertical split
                case 5:
                    shapeRenderer.rect(0, y0, LoreJourney.V_WIDTH, -LoreJourney.V_HEIGHT / 2);
                    shapeRenderer.rect(0, y1, LoreJourney.V_WIDTH, LoreJourney.V_HEIGHT / 2);
                    break;
            }
        }
