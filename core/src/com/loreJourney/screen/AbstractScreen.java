...
public abstract class AbstractScreen implements Screen {
    ...
    protected final LoreJourney game;
    ...
    public AbstractScreen(final LoreJourney game, final ResourceManager rm) {
        this.game = game;
        this.rm = rm;

        cam = new OrthographicCamera(LoreJourney.V_WIDTH, LoreJourney.V_HEIGHT);
        cam.setToOrtho(false);
        // the game will retain it's scaled dimensions regardless of resizing
        viewport = new StretchViewport(LoreJourney.V_WIDTH, LoreJourney.V_HEIGHT, cam);
    ...
    public LoreJourney getGame() { return game; }
    ...
    public void setSlideScreen(final Screen screen, boolean right) {
        if (clickable) {
            ...
            stage.addAction(Actions.sequence(
                Actions.moveBy(right ? -LoreJourney.V_WIDTH : LoreJourney.V_WIDTH, 0, 0.15f),
                Actions.run(new Runnable() {
                    @Override
                    public void run() {
                        clickable = true;
